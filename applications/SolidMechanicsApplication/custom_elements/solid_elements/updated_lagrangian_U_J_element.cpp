...
Element::Pointer UpdatedLagrangianUJElement::Clone(IndexType NewId, NodesArrayType const& rThisNodes) const
{
  ...
  return Kratos::make_shared<UpdatedLagrangianUJElement>(NewElement);
}
...
void UpdatedLagrangianUJElement::GetValuesVector(Vector& rValues, int Step)
{
  ...
  for(SizeType i=0; i<number_of_nodes; ++i)
  {
    ...
    if(dimension == 3)
    {
      ...
      rValues[index + 3] = GetGeometry()[i].FastGetSolutionStepValue(JACOBIAN, Step);
    }
    else
    {
      rValues[index + 2] = GetGeometry()[i].FastGetSolutionStepValue(JACOBIAN, Step);
    }
  }
}
...
void UpdatedLagrangianUJElement::CalculateStabilizationParameter(double& rStabilizationParameter, const ProcessInfo& rCurrentProcessInfo)
{
  KRATOS_TRY
  ...
  rStabilizationParameter *= StabilizationFactor;
  ...
  rStabilizationParameter /= LameMu;
  rStabilizationParameter *= BulkModulus;

  if(dimension == 2)
  {
    rStabilizationParameter /= 18.0;
  }
  else
  {
    rStabilizationParameter /= 80.0;
  }
...
void UpdatedLagrangianUJElement::GetValueOnIntegrationPoints(const Variable<Matrix>& rVariable, std::vector<Matrix>& rValue, const ProcessInfo& rCurrentProcessInfo)
{
  ...
  {
    this->CalculateOnIntegrationPoints(rVariable, rValue, rCurrentProcessInfo);
  }
...
void UpdatedLagrangianUJElement::CalculateOnIntegrationPoints(const Variable<double>& rVariable, std::vector<double>& rOutput, const ProcessInfo& rCurrentProcessInfo)
{
  ...
  else if(rVariable == POROSITY)
  {
    ...
    if(rOutput.size() != mConstitutiveLawVector.size())
      rOutput.resize(mConstitutiveLawVector.size(), false);
  ...
  else if(rVariable == PERMEABILITY)
  {
    ...
    if(Kozeny == false)
    {
      ...
      this->GetValueOnIntegrationPoints(POROSITY, Porosity, rCurrentProcessInfo);

      double PorosityInitial = GetProperties()[INITIAL_POROSITY];
      double initialVoidRatio = PorosityInitial/(1.0-PorosityInitial);

      double Constant = Permeability *(1.0+initialVoidRatio)/pow(initialVoidRatio,3);

      for(SizeType PointNumber=0; PointNumber<integration_points_number; ++PointNumber)
      {
        double voidRatio = Porosity[PointNumber] /(1.0-Porosity[PointNumber]);
        rOutput[PointNumber] = Constant * pow(voidRatio, 3)/(1.0 + voidRatio);
...
void UpdatedLagrangianUJElement::CalculateOnIntegrationPoints(const Variable<Vector>& rVariable, std::vector<Vector>& rOutput, const ProcessInfo& rCurrentProcessInfo)
{
  ...
  else if(rVariable == DARCY_FLOW)
  {
    ...
    //check WATER_PRESSURE dofs for one node
    if(GetGeometry()[0].HasDofFor(WATER_PRESSURE)){
      ...
      for(SizeType PointNumber=0; PointNumber<integration_points_number; ++PointNumber)
      {
        ...
        Vector GradP(dimension);
        noalias(GradP) = ZeroVector(dimension);
        ...
        for(SizeType i=0; i<number_of_nodes; ++i)
        {
          ...
          {
            GradP[j] += Variables.DN_DX(i, j) * rWaterPressure;
          }
        }
        ...
        // Manual resize
        Vector ResizedVector(3);
        noalias(ResizedVector) = ZeroVector(3);
        for(SizeType i=0; i<dimension; ++i) {
...
void UpdatedLagrangianUJElement::CalculateOnIntegrationPoints(const Variable<Matrix>& rVariable, std::vector<Matrix>& rOutput, const ProcessInfo& rCurrentProcessInfo)
{
  ...
  if(rVariable == CAUCHY_STRESS_TENSOR)
  {
    ...
    for(SizeType PointNumber=0; PointNumber<mConstitutiveLawVector.size(); ++PointNumber)
    {
      ...
      if((rOutput[PointNumber].size1() != 3) ||(rOutput[PointNumber].size2() != 3))
...
void UpdatedLagrangianUJElement::Initialize()
{
  ...
  if(mDeterminantF0.size() != integration_points_number)
    mDeterminantF0.resize(integration_points_number);
  ...
  if(mDeterminantJ0.size() != integration_points_number)
    mDeterminantJ0.resize(integration_points_number);
                                                    ...
                                                    double& rIntegrationWeight)
{
  ...
  for(SizeType i=0; i< number_of_nodes; ++i)
    ElementJacobian += GetGeometry()[i].FastGetSolutionStepValue(JACOBIAN) * rVariables.N[i];
